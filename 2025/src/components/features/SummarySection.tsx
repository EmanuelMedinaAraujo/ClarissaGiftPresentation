...
export const SummarySection: React.FC<SummarySectionProps> = ({
  ...
  // Ski item — only present on the Summary page
  const skiItem: PackingItem = {
    id: "ski",
    label: "Ski-Ausrüstung (Helm, Handschuhe, Skibrille)",
    icon: "Ski",
    anecdote:
      "Wir fahren Skifahren — bitte vergiss Helm, Handschuhe und Skibrille nicht! ⛷️",
  };

  const visiblePackingList: PackingItem[] = [...packingList, skiItem];
            ...
            >
              <div
                className={`text-slate-400 ${
                  checkedItems[item.id] ? "text-action-accent" : ""
                }`}
              >
      ...
      <section>
        ...
        <div className="space-y-3">
          {timelineData.map((event, index) => (
            <Card
              key={index}
              className="p-4 flex gap-4 items-start shadow-sm text-left"
            >
